    ...
    class WishpondPlugin {
        ...
        if (isset($_POST['submit']) && $_POST['submit']) {
          if (isset($_POST['token'])) {
            ...
            if ($token === get_option('wishpond_auth_token')) {
              $error_message = __('Token value is the same', 'slidein-popup') . ': ' . $token;
            } else {
              $saved = update_option('wishpond_auth_token', $token, '', 'no');

              if ($saved) {
                $notice_message = __('Successfully updated token', 'slidein-popup') . ': ' . $token;
              } else {
                $error_message = __('Unable to update token', 'slidein-popup') . ': ' . $token;
              }
            }
          }
        }
      ...
      public function publish_campaign($data) {
        if (!is_user_logged_in()) {
          wp_die(__('Not logged in', 'slidein-popup'));
        }

        if (!current_user_can('activate_plugins')) {
          wp_die(__('Not enough permissions', 'slidein-popup'));
        }

        if (!WishpondUtilities::permalink_structure_valid()) {
          $message = __('Invalid permalink structure. Please go to "Settings"->"Permalinks" and make sure your permalinks use the "postname"', 'slidein-popup');
          ...
          $action   = __('saved', 'slidein-popup');

          if ($campaign->post_id !== -1) {
            $saved  = $campaign->update($path);
            $action = __('updated', 'slidein-popup');
          } else {
            $saved = $campaign->save();
          }
          ...
          if ($saved) {
            ...
            $message = WishpondUtilities::json_message('updated', __('Campaign ', 'slidein-popup') . $action . __(' successfully. ', 'slidein-popup') . $campaign_url);
          } else {
            $message = WishpondUtilities::json_message('error', __('An unknown error occurred.', 'slidein-popup'));
          }
        }
      ...
      public function delete_campaign($data) {
        ...
        if ($campaign) {
          if ($campaign->remove()) {
            $message = WishpondUtilities::json_message('updated', __('Campaign removed successfully.', 'slidein-popup'));
          } else {
            $message = WishpondUtilities::json_message('error', __('An unknown error occurred.', 'slidein-popup'));
          }
        } else {
          $message = WishpondUtilities::json_message('error', __('Campaign not found.', 'slidein-popup'));
        }
      ...
      function wishpond_load_plugin_textdomain() {
        load_plugin_textdomain( 'slidein-popup', FALSE, basename( dirname( __FILE__ ) ) . '/lang/' );
      }
    }
  }
